...
const EditItem = ({item}) => {
    ...
    return(
        <Fragment>
            <button type="button" class="btn btn-warning" data-toggle="modal" data-target={`#id${item.item_id}`}>
            Edit
            </button>

            <div class="modal" id={`id${item.item_id}`} onClick={() => setDescription(item.description)}>
